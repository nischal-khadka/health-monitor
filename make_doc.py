...
spacer()
note("The requirements include bcrypt==4.0.1 which is required. If you see a bcrypt error when registering, make sure you ran pip install -r requirements.txt from inside the backend folder.")
...
body("You will be redirected to the login page. Click Register to create your account, then sign in. You will see the dashboard with Heart Rate, SpO2, and Temperature cards.")
...
spacer()
heading2("Step 18: Create Your First Account")
body("Open your browser and go to the register page:")
code("http://localhost:5173/register")
body("Fill in your username, email, and password then click Register. You will be redirected to login automatically.")
spacer()
body("After logging in, add your first patient via the API docs:")
code("http://localhost:8000/docs")
body("Use the POST /api/patients/ endpoint to add a patient (name, age, gender, ward).")

spacer()
heading2("Step 19: Test Without ESP32 (Optional)")
body("Send a fake reading to verify the full stack works (replace 1 with your actual patient ID):")
...
heading2("Registration fails with error")
bullet("Make sure you ran: pip install -r requirements.txt from inside the backend/ folder")
bullet("The file requires bcrypt==4.0.1 — newer versions break password hashing")
bullet("Restart the backend after installing dependencies")
